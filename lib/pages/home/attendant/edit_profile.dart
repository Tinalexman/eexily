...
class EditAttendantProfilePage extends ConsumerStatefulWidget {
  const EditAttendantProfilePage({super.key});

  @override
  ConsumerState<EditAttendantProfilePage> createState() =>
      _EditAttendantProfilePageState();
}

class _EditAttendantProfilePageState
    extends ConsumerState<EditAttendantProfilePage> {
  ...
  void initState() {
    ...
    firstNameController.text = attendant.firstName;
    lastNameController.text = attendant.lastName;
    addressController.text = attendant.address;
    storeController.text = attendant.gasStationName;
    accountNameController.text = attendant.accountName;
    accountNumberController.text = attendant.accountNumber;
  ...
  Future<void> updateAttendant() async {
    String id = ref.watch(userProvider.select((u) => u.id));
    var response = await updateAttendantUser(authDetails, id);
    ...
    ref.watch(userProvider.notifier).state = attendant.copyWith(
      firstName: firstName,
      lastName: lastName,
      gasStationName: storeName,
      address: address,
      bankName: bankName,
